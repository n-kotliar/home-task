import{d as q,h as B}from"./assets/header-C2sMTHIG.js";import{n as u}from"./assets/vendor-BP42vfHS.js";const i={filters:document.querySelector(".filters"),navButtons:document.querySelector(".nav-buttons"),musclesBtn:document.querySelector(".muscles-btn"),exercisesTitle:document.querySelector(".exercises-title"),searchForm:document.querySelector(".search-form"),loadMoreBtn:document.querySelector(".load-more-btn"),quoteContainer:document.querySelector(".quote"),pagination:document.querySelector(".pagination"),exercises:document.querySelector(".exercises-div")};let C=window.innerWidth<768?9:12,F=window.innerWidth<768?8:10,r=1,d="Muscles",b="Muscles",L="",y="",g="",p=[];q(i.quoteContainer);x();i.musclesBtn.classList.add("active-btn");i.filters.addEventListener("click",N);i.exercises.addEventListener("click",j);i.searchForm.addEventListener("input",I);var S;(S=i.loadMoreBtn)==null||S.addEventListener("click",loadMore);function f(e){return e[0].toUpperCase()+e.slice(1)}async function x(e=!0){e&&(r=1,i.exercises.innerHTML="");let t=`https://your-energy.b.goit.study/api/filters?filter=${d}&page=${r}&limit=${C}`;b.trim()&&(t+=`&name=${b}`);const a=await(await fetch(t)).json();if(!a.results.length){E();return}H(a.results),$(a.totalPages)}function H(e){const t=`
    <ul class="exercises">
